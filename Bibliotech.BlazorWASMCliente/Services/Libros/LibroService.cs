...
namespace Bibliotech.BlazorWASMCliente.Services.Libros
{
    public class LibroService : ILibrosService
    {
        ...
        public async Task<List<LibroDTO>> GetBookByTittle(string tittle)
        {

            var result = await _http.GetFromJsonAsync<ResponseApi<List<LibroDTO>>>($"api/Libros/GetBookByTittle/{tittle}");
            if (result!.Success)
                return result.Value;
            else
                throw new Exception(result.Message);
